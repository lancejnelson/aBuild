...
class MTP(object):
    ...
    def setup_train(self,trainingRoot,species):
        ...
        if dirs != []:
            ...
            trainingSet = dataset.from_paths(dirs,calculator='VASP')  #####
...
#                    #                    print(thisCrystal.appMinDist,' approp Min Dist')
    ...
    def setup_select(self):
        ...
        from shutil import copy
        from glob import glob
        from aBuild.utility import cat,grep
        from aBuild.fitting.mtp import MTP
        ...
        nCandidates = int(grep(candidatesFile,'begin','-i','|wc -l')[-1])
        print(nCandidates, 'Found this many candidates')
        if nCandidates > 2e6:
            msg.info("Found lots of candidates, splitting into two files")
            splitLine = grep(candidatesFile,'begin','-in')[nCandidates /2].split(':')[0]
            totalLines = cat(candidatesFile,' | wc -l')
            firstFile = head(candidatesFile,splitLine, '> ' + path.join(self.root,'candidates_part1.cfg'))
            secondFile = tail(candidatesFile, totalLines - splitLine, ' >' + path.join(self.root,'candidates_part2.cfg'))
